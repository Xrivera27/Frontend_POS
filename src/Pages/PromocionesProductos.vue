        ...
        <div class="contenedor-interno contenedor-izquierdo">
          ...
          <div class="form-group">
            ...
            <input
              type="text"
              id="nombre_promocion"
              v-model="promForm.promocion_nombre"
          ...
          <div class="contenedor-interno">
            ...
            <div class="form-group">
              ...
              <input
                type="text"
                id="nombre_promocion"
                v-model="promFormModal.promocion_nombre"
                name="nombre_promocion"
                required
              />
            </div>
    ...
    <div class="modal" v-if="showConflictModal">
      <div class="modal-content">
        ...
        <div class="conflict-explanation">
          <p>
            Existe una promoción activa para el producto
            <strong
              >"{{
                tempPromocionData?.producto?.nombre ||
                tempPromocionData?.producto
              }}"</strong
            >
          ...
          <div class="promocion-details existing">
            ...
            <p>
              ...
              {{ conflictingPromocion?.promocion_nombre }}
            </p>
            <p>
          ...
          <div class="promocion-details new">
            ...
            <p>
              <strong>Nombre:</strong> {{ tempPromocionData?.promocion_nombre }}
            </p>
            <p>
              <strong>Descuento:</strong>
        ...
        <div class="modal-actions">
          ...
          <button class="btn btn-warning" @click="createAnywayPromocion">
            <i class="bi bi-check2-circle"></i> Activar esta y desactivar
            anterior
          </button>
        </div>
      </div>
...
export default {
    ...
    filterPromociones() {
      return this.promociones.filter(
        (promocion) =>
          ...
          (promocion.producto?.nombre || promocion.producto)
            ?.toLowerCase()
            .includes(this.searchQuery.toLowerCase())
      );
    },
  ...
  methods: {
    ...
    async cargarProductos() {
      this.isLoading = true;
      try {
        const productosResponse = await solicitudes.fetchRegistros(
          "/promocionesP/productos/empresa"
        );
    ...
    limpiarForm() {
      ...
      if (this.activeForm) {
        this.clearForm();
        this.$nextTick(() => {
          this.$refs.produInputCreate?.focus();
        });
      }
    },
        ...
        if (!this.isEditing) {
          try {
            ...
            if (error.response?.status === 409) {
              console.log("Conflicto detectado:", error.response.data);
    ...
    async activarProm(promocionId) {
      ...
      try {
        ...
        const promocionActivaMismoProducto = this.promociones.find(
          (p) =>
        ...
        if (promocionActivaMismoProducto) {
          ...
          notis(
            "warning",
            "Ya existe una promoción activa para este producto en las fechas seleccionadas"
          );
          return;
        }
    ...
    // Método auxiliar para verificar conflicto de fechas
    ...
    async createAnywayPromocion() {
      ...
      try {
        if (this.tempPromocionData) {
          // Si es una nueva promoción (no tiene ID)
          if (!this.tempPromocionData.id) {
            // Crear la nueva promoción con force_create
            const datos = {
            ...
            // Es una promoción existente, usar el flujo de activación
    ...
    editarPromocion(index) {
      ...
      this.showModal();
      this.$nextTick(() => {
        this.$refs.produInputEdit?.focus();
      });
    },
    ...
    closeConflictModal() {
      if (this.tempPromocionData && this.tempPromocionData.id) {
        ...
        this.promFormModal = {
          ...
          producto_id: this.tempPromocionData.producto_id,
          producto:
            this.tempPromocionData.producto?.nombre ||
            this.tempPromocionData.producto,
          promocion_nombre: this.tempPromocionData.promocion_nombre,
          porcentaje_descuento: this.tempPromocionData.porcentaje_descuento,
